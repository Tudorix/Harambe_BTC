...
public class Extendo {
    ...
    //Singleton
    private static Extendo single_instance = null;
    Thread thread = null;
    private boolean running = false;

    public Extendo(HardwareClass hardwareClass, Telemetry telemetry){
    ...
    public int getPosition() throws InterruptedException{
    ...
    public static synchronized Extendo getInstance(HardwareMap hardwareMap, Telemetry telemetry){
        if(single_instance == null){
            single_instance = new Extendo(HardwareClass.getInstance(hardwareMap), telemetry);
        }
